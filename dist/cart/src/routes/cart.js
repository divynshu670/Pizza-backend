...
router.post("/", authMiddleware, body("pizzaId").notEmpty(), body("quantity").isInt({ gt: 0 }), async (req, res) => {
...
router.put("/:id", authMiddleware, body("quantity").isInt({ gt: 0 }), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const { id } = req.params;
