...
router.get("/google/callback", passport.authenticate("google", { failureRedirect: "/login", session: true }), async (req, res) => {
    ...
    try {
        ...
        const token = createToken(user.id);
        // You can redirect to front-end with token in query or set a cookie.
        // For simplicity, redirect to a route that can read the token in query
        // (Change to your front-end URL). Example:
